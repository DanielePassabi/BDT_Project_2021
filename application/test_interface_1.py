...
class UpdateDBInterface(wx.Frame):
    ...
    def loadWidgets(self, panel):
        ...
        #self.combobox_data_type.SetValue(choices[0])
        ...
        if all_data:
            ...
            try:
                if data_to_update == "CIG":
                    updateCIGTable(host, port, database, user, password, csv_path)

                elif data_to_update == "AGGIUDICATARI":
                    updateAggiudicatariTable(host, port, database, user, password, csv_path)
